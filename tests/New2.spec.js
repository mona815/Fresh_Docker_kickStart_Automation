...
test('test', async ({ page }) => {
  ...
  await page.getByLabel('First name').fill('yasmien');
  await page.getByLabel('Last name').click();
  await page.getByLabel('Last name').fill('Ahmed');
  await page.getByLabel('Email').click();
  await page.getByLabel('Email').fill('mona+9494@cloudypeida.net');
  await page.getByLabel('Phone number').click();
  await page.getByLabel('Phone number').fill('0564879768');
  await page.getByLabel('- Select -').click();
  await page.getByRole('option', { name: 'Organization 2' }).click();
  await page.locator('#edit-organization-profiles-0-entity-field-organization-categories-wrapper--AErGBw_qi9I').getByRole('combobox').click();
  await page.getByRole('option', { name: 'Geo / Location' }).click();
